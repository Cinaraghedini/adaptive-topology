...
% $Id: times.m,v 1.20 2007/05/15 14:33:59 joloef Exp $
...
%if isa(Y,'sdpvar')
    ...
    if m==1
        ...
        temp = X.*(reshape(Y.basis(:,1),n,m));
        X = X(:);
        if isequal(Y.conicinfo,[1 0]) & (n == m) & (length(Y.lmi_variables)==n*(n+1)/2)
            % X.*Y where Y is a standard symmetric primal cone variable 
            [i,j,k] = find((Y.basis).');
            y.basis = sparse(j,i,X.*k);
        else
            for i = 1:nv
                temp2 = X.*Y.basis(:,i+1);
                y.basis(:,i+1) = temp2(:);
