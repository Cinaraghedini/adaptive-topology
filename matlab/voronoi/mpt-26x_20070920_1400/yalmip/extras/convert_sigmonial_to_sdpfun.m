...
if any(model.variabletype > 3)
    ...
    for i = 1:length(sigmonials)
        ...
        if 0%isequal(sort(monoms(find(monoms))) , [-1 1])
            x = find(monoms == 1);
            y = find(monoms == -1);
            model.evalVariables = [model.evalVariables sigmonials(i)];
            model.evalMap{end+1}.fcn = 'fraction_internal2';
            model.evalMap{end}.arg{1} = recover([x y]);
            model.evalMap{end}.arg{2} = [];
            model.evalMap{end}.variableIndex = [x y];
            model.evalMap{end}.computes = sigmonials(i);
            model.evalMap{end}.variableIndex = [x y];
            model.evalMap{end}.properties.bounds = @fraction_bound;
            model.evalMap{end}.properties.convexhull = @fraction_convexhull;
            model.evalMap{end}.properties.derivative = @fraction_derivative;
            model.monomtable(sigmonials(i),:) = 0;
            model.monomtable(sigmonials(i),sigmonials(i)) = 1;
            model.variabletype(sigmonials(i)) = 0;
