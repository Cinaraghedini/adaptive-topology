...
% DESCRIPTION
% -------------------------------------------------------------------------
% [sep_hyp,spec]=hit_prec_SVC(X_or,F,s)
%
% -------------------------------------------------------------------------
% INPUT
% -------------------------------------------------------------------------
...
for i=1:s
    ...
    for j=i+1:s
        ...
        % Call SVC: the margin (C parameter in literature) is inf
        [nsv,alpha,b0]=svc(ClustBin_totx,ClustBin_toty,'linear',inf,idpar.QPsolver);

        % compute the separating hyperplane wij*x+b0=0
        dim_domain=size(X_or,2); % dimension of the domain

        wij=zeros(dim_domain,1);
        for k=1:ni+nj
            wij=wij+alpha(k)*ClustBin_totx(k,:)'*ClustBin_toty(k,:);
        end

        %
        % wij * x+b0 is > 0 on the points in the i-th class (marked with
        % +1)
        % Therefore signs of wij and b0 must be inverted to have
        % wij * x +b0 <0 0n the i-the class
        %
        %
        ...
        spec.correctness(i,j)= correctness(ClustBin_totx,ClustBin_toty,wij,b0);
        ...
        % the opposit sign to preserve  wji x < b on the j-th region
