...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
 if(nrhs < NPARIN)
   mexErrMsgTxt("blkmul requires more input arguments.");
 if (nlhs > NPAROUT)
   mexErrMsgTxt("blkmul generates 1 output argument.");
/* ------------------------------------------------------------
   Get inputs d, mu, NL
   ------------------------------------------------------------ */
 d = mxGetPr(D_IN);
 ...
 if(mxGetM(NL_IN) * mxGetN(NL_IN) != kappa)
   mexErrMsgTxt("nL size mismatch");
 nLpr = mxGetPr(NL_IN);
/* ------------------------------------------------------------
