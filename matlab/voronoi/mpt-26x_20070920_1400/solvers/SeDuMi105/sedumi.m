...
%
%    (4) pars.stepdif, pars.w. By default, stepdif = 0 and w=[1 1]. You can
%      enable primal/dual step length differentiation by setting stepdif=1.
...
%
% Bug reports can be sent to j.f.sturm@kub.nl
%
% SEE ALSO mat, vec, cellK, eyeK AND eigK.
...
% ----------------------------------------
   ...
   elseif (abs(1+feasratio) < 0.05) & (by > 0) & (R.b0*y0 < 0.5)
...
% --------------------------------------------------
   r0 = sum(R.w);
   if y0 * r0 < pars.eps * (1+x0)      % P/D residuals small
     cx = by + y0*R.sd - x0 / d.l(1);
     rgap = max(cx-by,0) / max([abs(cx),abs(by),1e-3 * x0]);
     if y0 * r0 + rgap < pars.eps * x0   %Approx feasible and optimal
       STOP = 1;
       break
...
% ------------------------------------------------------------
 if x0 > 0
   x = x / x0; y = y / x0;
   pinf = pinf /x0; dinf = dinf / x0;
   cx = cx/ x0; by = by / x0;
   normx = normx / x0; normy = normy / x0;
