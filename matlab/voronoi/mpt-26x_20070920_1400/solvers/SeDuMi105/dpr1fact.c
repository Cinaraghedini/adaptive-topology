...
/* ------------------------------------------------------------
  ...
  if( mxGetM(SMULT_IN) * mxGetN(SMULT_IN) != n)             /* smult */
    mexErrMsgTxt("Size mismatch smult.");
  smult = mxGetPr(SMULT_IN);
  maxu = mxGetScalar(MAXU_IN);                        /* maxu */
/* ------------------------------------------------------------
   DISASSEMBLE structure Lsymb.{dz,perm,first}
   ------------------------------------------------------------ */
  if(!mxIsStruct(LSYMB_IN))
    mexErrMsgTxt("Lsymb should be a structure.");
  if( (MY_FIELD = mxGetField(LSYMB_IN,0,"dz")) == NULL)   /* Lsymb.dz */
    mexErrMsgTxt("Missing field Lsymb.dz.");
  if(mxGetM(MY_FIELD) != m || mxGetN(MY_FIELD) != n)
    mexErrMsgTxt("Lsymb.dz size mismatch.");
  if(!mxIsSparse(MY_FIELD))
    mexErrMsgTxt("Lsymb.dz must be sparse.");
  dz.jc = mxGetJc(MY_FIELD);
  dz.ir = mxGetIr(MY_FIELD);                             /* (rowperm) */
  if( (MY_FIELD = mxGetField(LSYMB_IN,0,"perm")) == NULL)   /* Lsymb.perm */
    mexErrMsgTxt("Missing field Lsymb.perm.");
  if( mxGetM(MY_FIELD) * mxGetN(MY_FIELD) != n)            /* (colperm) */
    mexErrMsgTxt("Size mismatch Lsymb.perm.");
  colpermPr = mxGetPr(MY_FIELD);
  if( (MY_FIELD = mxGetField(LSYMB_IN,0,"first")) == NULL)   /* Lsymb.first */
    mexErrMsgTxt("Missing field Lsymb.first.");
  if( mxGetM(MY_FIELD) * mxGetN(MY_FIELD) != n)
    mexErrMsgTxt("Size mismatch Lsymb.first.");
