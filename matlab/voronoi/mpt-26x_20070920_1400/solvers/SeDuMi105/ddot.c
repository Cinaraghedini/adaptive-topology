...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
  if(nrhs < NPARINMIN)
    mexErrMsgTxt("ddot requires more input arguments.");
  if (nlhs > NPAROUT)
    mexErrMsgTxt("ddot generates less output arguments.");
/* ------------------------------------------------------------
  ...
  if((nblk = mxGetM(BLKSTART_IN) * mxGetN(BLKSTART_IN) - 1) < 0)
  ...
  if(!mxIsSparse(X_IN)){
    if(nrows != qDim)
      if(nrows < blkstart[nblk])
        if(nrows == nblk + qDim)         /* Lorentz tr + norm bound */
          X.pr += nblk;
        else
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
    if(nrhs < NPARIN)
      mexErrMsgTxt("ddot with sparse X requires more input arguments.");
    if(mxGetM(XBLKJC_IN) != m || mxGetN(XBLKJC_IN) < 3)
      mexErrMsgTxt("Xjc size mismatch");
